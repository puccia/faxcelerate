...
hovertipShowUnderMouse = function(el, o) {
  str = "";
  for (prop in el.ht.tip) {
	str += prop + ", ";
}
  hovertipHideCancel(el);
  if (!hovertipIsVisible(el)) {
	if (el.ht.tip.attr("tiploader") != undefined) {
		f= eval(el.ht.tip.attr("tiploader"));
		f(el.ht.tip, el);
	}
